...
class HackerNewsDriver(BaseWebDriver):

    def __init__(self, *args, **kwargs):
        options = {'images': False, 'javascript': True}
        super(HackerNewsDriver, self).__init__(browser_cls=Browser,
                                               options=options,
                                               *args,
                                               **kwargs)

    def step_1(self, html=None):
        print 'Loading hackernews'
        headers = {"Accept": "*/*",
    ...
    def step_2(self, html=None):
        print 'Getting login form'
        self.browser.click('a[href^="newslogin"]')

    def step_3(self, html=None):
        print 'Entering login data'
        self.browser.fill_input('input[type="text"][name="u"]',
                                'myusername')
        self.browser.fill_input('input[type="password"][name="p"]',
                                'mypassword')
        self.browser.click('input[type="submit"][value="login"]')

    def step_4(self, html=None):
        print 'Logging out'
        self.browser.click('a[href^="/r?fnid"]')
